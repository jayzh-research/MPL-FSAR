...
from models.base.vision_lstm import VisionLSTM
...
def self_distribution(x):
    distance_matrix = torch.empty((x.size(0), x.size(1), x.size(1)))
    for j in range(x.size(0)):
        for i in range(x.size(1)):
            distance_matrix[j][i] = x[j][i] - x[j]
    return distance_matrix

def calculate_divergence_loss(dist1, dist2):
    dist_tensor1 = self_distribution(dist1)
    dist_tensor2 = self_distribution(dist2)
    #dist_tensor3 = self_distribution(dist3)
    divergence_loss = F.mse_loss(dist_tensor1, dist_tensor2)
...
def hausdorff_dist_window(dists1, window_size=3):
    """
    Calculates the Hausdorff distance ensuring that the minimum index
    in dimension 3 does not differ by more than 2 from the corresponding index in dimension 2.

    :param dists: A tensor of shape (n_queries, n_support, query_seq_len, support_seq_len)
    :return: Cumulative distances satisfying the index constraint.
    """
    dists2 = dists1.clone()
    n_queries, n_support, query_seq_len, support_seq_len = dists1.shape

    while True:
        # 计算在维度3上的最小值和对应的索引
        min_values, min_indices = dists1.min(dim=3)

        # 创建支持序列索引的张量
        support_indices = torch.arange(support_seq_len).to(dists1.device).unsqueeze(0).unsqueeze(0)

        # 创建一个布尔掩码，检查索引差值
        index_difference = min_indices - support_indices
        valid_mask = (index_difference <= window_size) & (index_difference >= -window_size)
        ...
        # 更新距离张量，排除无效的最小值
        for i in range(torch.nonzero(~valid_mask).size(0)):
            j = torch.nonzero(~valid_mask)[i][0]
            k = torch.nonzero(~valid_mask)[i][1]
            m = torch.nonzero(~valid_mask)[i][2]
            dists1[j][k][m][min_indices[j][k][m]] = float('inf')  # 对于不满足条件的，设置为无穷大

    # 计算累积距离
    cum_dists1 = min_values.sum(dim=2)

    while True:
        # 计算在维度3上的最小值和对应的索引
        min_values, min_indices = dists2.min(dim=2)

        # 创建支持序列索引的张量
        support_indices = torch.arange(support_seq_len).to(dists2.device).unsqueeze(0).unsqueeze(0)
        ...
        for i in range(torch.nonzero(~valid_mask).size(0)):
            ...
            k = torch.nonzero(~valid_mask)[i][1]
            m = torch.nonzero(~valid_mask)[i][2]
            dists2[j][k][min_indices[j][k][m]][m] = float('inf')  # 对于不满足条件的，设置为无穷大

    # 计算累积距离
    cum_dists2 = min_values.sum(dim=2)

    return cum_dists1 + cum_dists2, dists1, dists2
...
class SelfAttentionAggregator(nn.Module):
    """
    对同一位置的多个样本进行自注意力聚合
    输入：形状为 (num_samples, feature_dim)
    输出：聚合后的特征向量 (feature_dim,)
    """
    ...
    def forward(self, x):
        # x: (N, d)  N=num_samples, d=feature_dim
        Q = self.query(x)  # (N, d)
        K = self.key(x)    # (N, d)
        V = self.value(x)  # (N, d)
        attn_scores = torch.matmul(Q, K.transpose(0, 1)) / self.scale  # (N, N)
        attn_weights = F.softmax(attn_scores, dim=1)  # (N, N)
        aggregated = torch.matmul(attn_weights, V)  # (N, d)
        # 为减少样本间噪音，取所有样本聚合结果的平均作为最终输出
        output = aggregated.mean(dim=0)  # (d,)
        return aggregated

class SlidingWindowPrototypeGenerator(nn.Module):
    """
    通过滑动窗口跨样本注意力聚合生成类别原型
    输入 support_features 的形状为 (num_classes, num_samples, num_frames, feature_dim)
    输出原型的形状为 (num_classes, num_frames, feature_dim)

    具体思路：设窗口大小 win = num_samples（此处=5），对每个类别，
    以滑动窗口方式沿帧维度提取连续win帧，对于窗口内每个相对位置，
    对所有样本该帧的特征用自注意力聚合生成局部原型；然后对每个帧位置，
    对所有窗口覆盖该帧的聚合结果取平均得到最终原型。
    """

    def __init__(self, feature_dim, window_size):
        super(SlidingWindowPrototypeGenerator, self).__init__()
        self.feature_dim = feature_dim
        self.window_size = window_size  # win=5
        self.aggregator = SelfAttentionAggregator(feature_dim)

    def forward(self, support_features):
        # support_features: (num_classes, num_samples, num_frames, feature_dim)
        num_classes, num_samples, num_frames, feature_dim = support_features.shape
        win = self.window_size
        num_windows = num_frames - win + 1  # 例如：8-5+1 = 4

        # 存储每个类别每个窗口的聚合结果
        # shape: (num_classes, num_windows, window_size, feature_dim)
        window_outputs = torch.zeros(num_classes, num_windows, win, feature_dim, device=support_features.device)

        for c in range(num_classes):
            # 对于每个类别
            for w in range(num_windows):
                # w窗口：从帧 w 到 w+win-1
                # 提取该类别所有样本在窗口内的特征: shape (num_samples, win, feature_dim)
                window_feat = support_features[c, :, w:w + win, :]  # (num_samples, win, feature_dim)
                # 对于窗口内每个相对位置 j (0到win-1)进行聚合
                for j in range(win):
                    # 对于每个样本的第j帧： shape (num_samples, feature_dim)
                    frame_feats = window_feat[:, j, :]
                    # 利用自注意力聚合这些样本特征
                    aggregated = self.aggregator(frame_feats)  # (feature_dim,)
                    window_outputs[c, w, j, :] = aggregated

        # 接下来，需要将多个窗口的结果融合成最终每个类别的原型
        # 对于每个类别和每个帧位置 i (0到num_frames-1)，统计所有窗口中覆盖该帧的位置
        # 窗口w覆盖帧位置 i if w <= i <= w+win-1.
        prototypes = torch.zeros(num_classes, num_frames, feature_dim, device=support_features.device)
        counts = torch.zeros(num_frames, device=support_features.device)  # 用于记录覆盖次数

        # 计算每个帧位置的聚合
        for i in range(num_frames):
            # 收集所有窗口w使得 w <= i <= w+win-1
            agg_list = []
            for w in range(num_windows):
                if w <= i <= w + win - 1:
                    pos = i - w  # 在窗口中的位置
                    agg_list.append(window_outputs[:, w, pos, :])  # shape: (num_classes, feature_dim)
            # 将列表中所有窗口对每个类别的输出求平均
            # agg_list: list of tensors, each (num_classes, feature_dim)
            agg_stack = torch.stack(agg_list, dim=0)  # (num_valid_windows, num_classes, feature_dim)
            proto_i = agg_stack.mean(dim=0)  # (num_classes, feature_dim)
            prototypes[:, i, :] = proto_i
        return prototypes


class TransformerEncoderBlock(nn.Module):
    """
    一个简单的Transformer Encoder块，用于自注意力聚合，
    输入和输出形状均为 (B, seq_len, d_model)
    """
    ...
    def forward(self, x):
        ...
        return x
class SlidingWindowPrototypeGenerator2(nn.Module):
    """
       通过滑动窗口，将每个类别中所有样本在连续窗口内的帧（5*5=25帧）一次性送入自注意力模块，
       得到与输入形状相同的聚合输出，然后将所有覆盖某一帧位置的窗口输出取平均，
       生成最终的类别原型。

       输入：support_features 的形状为 (num_classes, num_samples, num_frames, feature_dim)
       输出：prototypes 的形状为 (num_classes, num_frames, feature_dim)
       """

    def __init__(self, feature_dim, window_size, num_heads=8):
        super(SlidingWindowPrototypeGenerator2, self).__init__()
        self.feature_dim = feature_dim
        self.window_size = window_size  # win = num_samples, 此处设为5
        #self.transformer = TransformerEncoderBlock(d_model=feature_dim, num_heads=num_heads)
        self.transformer = MultiHeadAttention(8, self.feature_dim, self.feature_dim // 8, self.feature_dim // 8,dropout=0.2)

    def forward(self, support_features):
        # support_features: (num_classes, num_samples, num_frames, feature_dim)
        num_classes, num_samples, num_frames, feature_dim = support_features.shape
        win = self.window_size
        num_windows = num_frames - win + 1  # 滑动窗口个数

        # 存储每个类别每个窗口的自注意力聚合结果
        # 目标形状: (num_classes, num_windows, num_samples*win, feature_dim)
        window_outputs = torch.zeros(num_classes, num_windows, num_samples * win, feature_dim,
                                     device=support_features.device)

        for c in range(num_classes):
            for w in range(num_windows):
                # 对于类别 c，在窗口w内，提取所有样本连续win帧特征：形状 (num_samples, win, feature_dim)
                window_feat = support_features[c, :, w:w + win, :]  # (5, win, feature_dim)
                # 拉直得到 (num_samples*win, feature_dim)，此处为 (25, feature_dim)
                flat_feat = window_feat.reshape(num_samples * win, feature_dim)
                # 添加batch维度：变为 (1, 25, feature_dim)
                flat_feat = flat_feat.unsqueeze(0)
                # 送入自注意力模块，输出形状依然为 (1, 25, feature_dim)
                agg_flat = self.transformer(flat_feat,flat_feat,flat_feat)
                # 移除batch维度，得到 (25, feature_dim)
                agg_flat = agg_flat.squeeze(0)
                # 存储输出
                window_outputs[c, w] = agg_flat

        # 最后融合所有窗口的结果，生成最终类别原型
        # 对于每个类别和每个帧位置 i (0 <= i < num_frames)，
        # 收集所有窗口w使得 w <= i < w+win，然后对这些窗口对应位置的聚合结果取平均
        prototypes = torch.zeros(num_classes, num_frames, feature_dim, device=support_features.device)
        for i in range(num_frames):
            # agg_list: list of tensor，每个tensor形状 (num_classes, feature_dim)
            agg_list = []
            for w in range(num_windows):
                if w <= i < w + win:
                    pos = i - w  # 在窗口中的位置
                    # window_outputs[:, w]的形状为 (num_classes, num_samples*win, feature_dim)
                    # 需要先恢复为 (num_classes, num_samples, win, feature_dim)
                    window_w = window_outputs[:, w].reshape(num_classes, num_samples, win, feature_dim)
                    # 对于每个类别，取所有样本在窗口中该位置的输出，形状为 (num_classes, num_samples, feature_dim)
                    window_pos = window_w[:, :, pos, :]
                    # 对样本维度取平均：得到 (num_classes, feature_dim)
                    window_pos_mean = window_pos.mean(dim=1)
                    agg_list.append(window_pos_mean)
            # 将所有窗口覆盖该帧位置的结果取平均：得到 (num_classes, feature_dim)
            agg_stack = torch.stack(agg_list, dim=0)
            proto_i = agg_stack.mean(dim=0)
            prototypes[:, i, :] = proto_i
...
class CNN_OTAM_CLIPFSAR(CNN_FSHead):
    ...
    def __init__(self, cfg):
        ...
        if cfg.VIDEO.HEAD.BACKBONE_NAME=="RN50":
            backbone, self.preprocess = load(cfg.VIDEO.HEAD.BACKBONE_NAME, device=cfg.gpu_id, cfg=cfg, jit=False)   # ViT-B/16
            ...
            backbone, self.preprocess = load(cfg.VIDEO.HEAD.BACKBONE_NAME, device=cfg.gpu_id, cfg=cfg,
                                             jit=False)  # ViT-B/16
            self.backbone = backbone.visual  # model.load_state_dict(state_dict)
            self.class_real_train = cfg.TRAIN.CLASS_NAME
            self.class_real_test = cfg.TEST.CLASS_NAME
        ...
        with torch.no_grad():
            ...
            text_templete = tokenize(text_templete).to(self.args.gpu_id)
...
@HEAD_REGISTRY.register()
class FEW_PROMPT_CLIPFSAR(CNN_FSHead):
    """
    OTAM with a CNN backbone.
    """

    def __init__(self, cfg):
        super(FEW_PROMPT_CLIPFSAR, self).__init__(cfg)
        args = cfg
        self.args = cfg
        if cfg.VIDEO.HEAD.BACKBONE_NAME == "RN50":
            backbone, self.preprocess = load(cfg.VIDEO.HEAD.BACKBONE_NAME, device=cfg.gpu_id, cfg=cfg,
            ...
            # self.backbone = CLIP
            self.mid_dim = 512
        self.shots = self.args.TRAIN.SHOT
        with torch.no_grad():
            # —— Train 类别文本特征 ——
            train_feats = []
            for action in self.class_real_train:
                descs = self.prompts_map[action]
                # 随机抽样 K 条
                sampled = random.sample(descs, min(self.shots, len(descs)))
                # tokenize，encode，mean pooling
                txt = tokenize(sampled).to(cfg.gpu_id)  # [K, L]
                emb = backbone.encode_text(txt)  # [K, D]
                emb = emb.mean(dim=0, keepdim=True)  # [1, D]
                train_feats.append(emb)
            # 拼成 [N_train, D]
            self.text_features_train = torch.cat(train_feats, dim=0)
    ...
    def forward(self, inputs):
        support_images, support_labels, target_images, support_real_class = inputs['support_set'], inputs[
            'support_labels'], inputs['target_set'], inputs[
            'real_support_labels']  # [200, 3, 224, 224] inputs["real_support_labels"]
        ...
        class_dists = [torch.mean(torch.index_select(cum_dists, 1, extract_class_indices(unique_labels, c)), dim=1) for
                       c in unique_labels]
        class_dists = torch.stack(class_dists)
        class_dists = rearrange(class_dists, 'c q -> q c')
        return_dict = {'logits': - class_dists, "class_logits": class_text_logits}
...
@HEAD_REGISTRY.register()
class MY2_CNN_OTAM_CLIPFSAR(CNN_FSHead):
    """
    OTAM with a CNN backbone.
    """

    def __init__(self, cfg):
        super(MY2_CNN_OTAM_CLIPFSAR, self).__init__(cfg)
        args = cfg
        self.args = cfg
        self.relu1 = nn.ReLU(inplace=True)
        if cfg.VIDEO.HEAD.BACKBONE_NAME == "RN50":
        ...
        self.deep_conv_layer = nn.Conv1d(in_channels=self.mid_dim, out_channels=self.mid_dim//self.args.DATA.NUM_INPUT_FRAMES, kernel_size=1)
    ...
    def get_OTAM_dists(self, support_features, target_features):
        n_queries = target_features.shape[0]
        n_support = support_features.shape[0]
    ...
    def get_hausdorff_dists(self, support_features, target_features):
        n_queries = target_features.shape[0]
        n_support = support_features.shape[0]
        frame_num = support_features.shape[1]

        support_features = rearrange(support_features.unsqueeze(0), 'b h s d -> b (h s) d')
        frame_sim = torch.matmul(F.normalize(support_features, dim=2),
                                 F.normalize(target_features, dim=2).permute(0, 2, 1)).reshape(n_queries, n_support,
                                                                                               frame_num, frame_num)
    ...
    def forward(self, inputs):
        ...
        if self.training:
            ...
            pre_dists = self.get_OTAM_dists(support_features, target_features)
            nearest_support_idx = torch.argmin(pre_dists, dim=1)
            nearest_support_idx_temp = copy.deepcopy(nearest_support_idx)
            cnt = 0
            #label_correction
            # while (len(torch.unique(nearest_support_idx)) != len(nearest_support_idx)) and cnt < 1:
            #     # 找到重复的标签及其索引
            #     unique_idx, counts = torch.unique(nearest_support_idx, return_counts=True)
            #     duplicate_idx = unique_idx[counts > 1]
            #     unique_len = len(counts) - len(duplicate_idx)
            #     for idx in duplicate_idx:
            #         # 找到重复标签的位置
            #         dup_indices = torch.nonzero(nearest_support_idx == idx).squeeze()
            #         # 根据距离重新分配标签
            #         max_row, max_index = -99, 0
            #         for i in range(len(dup_indices)):
            #             row = torch.min(pre_dists[dup_indices[i]])
            #             if row > max_row:
            #                 max_row = row
            #                 max_index = dup_indices[i]
            #                 # row2 = torch.min(pre_dists[dup_indices[i+1]])
            #         sorted_indices = torch.argsort(pre_dists[max_index])
            #         nearest_support_idx[max_index] = sorted_indices[1]
            #         unique_idx, counts = torch.unique(nearest_support_idx, return_counts=True)
            #         duplicate_idx = unique_idx[counts > 1]
            #         if unique_len > len(counts) - len(duplicate_idx):
            #             nearest_support_idx = nearest_support_idx_temp
            #         if len(torch.unique(nearest_support_idx)) == len(nearest_support_idx):
            #             break
            #     if unique_len > len(counts) - len(duplicate_idx):
            #         nearest_support_idx = nearest_support_idx_temp
            #     if len(torch.unique(nearest_support_idx)) == len(nearest_support_idx):
            #         break
            #     cnt = cnt + 1
            assigned_target_real_class = support_real_class[nearest_support_idx]

            if self.training:
                context_support = self.text_features_train[support_real_class.long()].unsqueeze(1)  # .repeat(1, self.args.DATA.NUM_INPUT_FRAMES, 1)
                context_target = self.text_features_train[assigned_target_real_class.long()].unsqueeze(1)

            else:
                context_support = self.text_features_test[support_real_class.long()].unsqueeze(1)
                context_target = self.text_features_test[assigned_target_real_class.long()].unsqueeze(1)
            ...
            # hybrid modulation text and frame
            s_features_in = torch.cat([support_features, context_support], dim=1)
            s_features_out = self.context2(s_features_in, s_features_in, s_features_in)
            support_features = s_features_out[:, :self.args.DATA.NUM_INPUT_FRAMES, :]
            context_support = s_features_out[:, self.args.DATA.NUM_INPUT_FRAMES:, :]

            t_features_in = torch.cat([target_features, context_target], dim=1)
            t_features_out = self.context2(t_features_in, t_features_in, t_features_in)
            target_features = t_features_out[:, :self.args.DATA.NUM_INPUT_FRAMES, :]
            context_target = t_features_out[:, self.args.DATA.NUM_INPUT_FRAMES:, :]

            #deep_conv
            s_conv_output = self.relu1(self.deep_conv_layer(support_features.permute(0, 2, 1)))
            deep_s_features = s_conv_output.contiguous().view(s_conv_output.size(0), 1, -1)

            t_conv_output = self.relu1(self.deep_conv_layer(target_features.permute(0, 2, 1)))
            deep_t_features = t_conv_output.contiguous().view(t_conv_output.size(0), 1, -1)

            #clip_loss
            #normalize
            deep_s_features = F.normalize(deep_s_features, p=2, dim=-1).squeeze(1)
            deep_t_features = F.normalize(deep_t_features, p=2, dim=-1).squeeze(1)
            context_support = F.normalize(context_support, p=2, dim=-1).squeeze(1)
            context_target = F.normalize(context_target, p=2, dim=-1).squeeze(1)
            #
            logits_ss = torch.matmul(deep_s_features, context_support.t())
            logits_tt = torch.matmul(deep_t_features, context_target.t())
            #
            labels_ss = torch.arange(logits_ss.size(0)).to(deep_s_features.device)
            loss_image_to_text_ss = F.cross_entropy(logits_ss, labels_ss)
            loss_text_to_image_ss = F.cross_entropy(logits_ss.t(), labels_ss)

            labels_tt = torch.arange(logits_tt.size(0)).to(deep_t_features.device)
            loss_image_to_text_tt = F.cross_entropy(logits_tt, labels_tt)
            loss_text_to_image_tt = F.cross_entropy(logits_tt.t(), labels_tt)

            clip_loss = (loss_image_to_text_ss + loss_text_to_image_ss + loss_image_to_text_tt + loss_text_to_image_tt)/4

            #hybrid modulation deep and frame
            # s_features_in = torch.cat([support_features, deep_s_features], dim=1)
            # s_features_out = self.context2(s_features_in, s_features_in, s_features_in)
            # support_features = s_features_out[:, :self.args.DATA.NUM_INPUT_FRAMES, :]
            # deep_s_features = s_features_out[:, self.args.DATA.NUM_INPUT_FRAMES:, :]
            #
            # t_features_in = torch.cat([target_features, deep_t_features], dim=1)
            # t_features_out = self.context2(t_features_in, t_features_in, t_features_in)
            # target_features = t_features_out[:, :self.args.DATA.NUM_INPUT_FRAMES, :]
            # deep_t_features = t_features_out[:, self.args.DATA.NUM_INPUT_FRAMES:, :]
            ...
            #
            # deep_s_features = [
            #     torch.mean(torch.index_select(deep_s_features, 0, extract_class_indices(support_labels, c)), dim=0)
            #     for c in unique_labels]
            # deep_s_features = torch.stack(deep_s_features)

            #calculate_divergence_loss
            cum_dists_OTAM = self.get_OTAM_dists(support_features, target_features)
            cum_dists_hausdorff = self.get_hausdorff_dists(support_features, target_features)
            total_cum_dists = cum_dists_hausdorff + cum_dists_OTAM
            # cum_dists_context = self.get_dists(context_support, context_target)
            # cum_dists_deep = self.get_dists(deep_s_features, deep_t_features)
            divergence_loss = calculate_divergence_loss(cum_dists_OTAM, cum_dists_hausdorff)
            ...
            if hasattr(self.args.TRAIN, "EVAL_TEXT") and self.args.TRAIN.EVAL_TEXT:
                ...
                #deep_conv
                # s_conv_output = self.deep_conv_layer(support_features.permute(0, 2, 1))
                # deep_s_features = s_conv_output.contiguous().view(s_conv_output.size(0), 1, -1)
                #
                # t_conv_output = self.deep_conv_layer(target_features.permute(0, 2, 1))
                # deep_t_features = t_conv_output.contiguous().view(t_conv_output.size(0), 1, -1)
                ...
                cum_dists_OTAM = self.get_OTAM_dists(support_features, target_features)
                cum_dists_hausdorff = self.get_hausdorff_dists(support_features, target_features)
                total_cum_dists = cum_dists_OTAM + cum_dists_hausdorff
                divergence_loss = None
                clip_loss = None
                # cum_dists_context = self.get_dists(context_support, context_target)
                # cum_dists_deep = self.get_dists(deep_s_features, deep_t_features)
                # divergence_loss = calculate_divergence_loss(cum_dists_frame, cum_dists_context, cum_dists_deep)
                #.......
                #cum_dists_frame = cum_dists_frame + cum_dists_context + cum_dists_deep

        class_dists_frame = [torch.mean(torch.index_select(total_cum_dists, 1, extract_class_indices(unique_labels, c)), dim=1) for
                       c in unique_labels]
        class_dists_frame = torch.stack(class_dists_frame)
        class_dists_frame = rearrange(class_dists_frame, 'c q -> q c')

        # class_dists_context = [torch.mean(torch.index_select(cum_dists_context, 1, extract_class_indices(unique_labels, c)), dim=1) for
        #     c in unique_labels]
        # class_dists_context = torch.stack(class_dists_context)
        # class_dists_context = rearrange(class_dists_context, 'c q -> q c')
        #
        # class_dists_deep = [
        #     torch.mean(torch.index_select(cum_dists_deep, 1, extract_class_indices(unique_labels, c)), dim=1) for
        #     c in unique_labels]
        # class_dists_deep = torch.stack(class_dists_deep)
        # class_dists_deep = rearrange(class_dists_context, 'c q -> q c')

        return_dict = {'logits': - class_dists_frame, "class_logits": class_text_logits, "clip_loss": clip_loss,
...
@HEAD_REGISTRY.register()
class MY_CNN_OTAM_CLIPFSAR(CNN_FSHead):
    """
    OTAM with a CNN backbone.
    """

    def __init__(self, cfg):
        super(MY_CNN_OTAM_CLIPFSAR, self).__init__(cfg)
        args = cfg
        self.args = cfg
        self.prompt_arr = ["first frame", "second frame", "third frame", "fourth frame", "fifth frame", "sixth frame",
                           "seventh frame", "eighth frame"]
        if cfg.VIDEO.HEAD.BACKBONE_NAME == "RN50":
            backbone, self.preprocess = load(cfg.VIDEO.HEAD.BACKBONE_NAME, device=cfg.gpu_id, cfg=cfg,
                                             jit=False)  # ViT-B/16
            self.backbone = backbone.visual  # model.load_state_dict(state_dict)
        ...
        with torch.no_grad():
            if hasattr(self.args.TEST, "PROMPT") and self.args.TEST.PROMPT:
                ...
                self.text_features_train_fra_list = []
                for fra in range(len(self.class_real_train)):
                    text_templete = ["{} of ".format(self.prompt_arr[int(ii)]) for ii in
                                     range(cfg.DATA.NUM_INPUT_FRAMES)]
                    for jj in range(len(text_templete)):
                        text_templete[jj] = text_templete[jj] + self.class_real_train[fra]
                    text_templete = tokenize(text_templete).to(self.args.gpu_id)
                    self.text_features_train = backbone.encode_text(text_templete)
                    self.text_features_train_fra_list.append(self.text_features_train)
                self.text_features_train = torch.stack(self.text_features_train_fra_list)
            ...
            if hasattr(self.args.TEST, "PROMPT") and self.args.TEST.PROMPT:
                ...
                self.text_features_test_fra_list = []
                for fra in range(len(self.class_real_test)):
                    text_templete = ["{} of ".format(self.prompt_arr[int(ii)]) for ii in
                                     range(cfg.DATA.NUM_INPUT_FRAMES)]
                    for jj in range(len(text_templete)):
                        text_templete[jj] = text_templete[jj] + self.class_real_test[fra]
                    text_templete = tokenize(text_templete).to(self.args.gpu_id)
                    self.text_features_test = backbone.encode_text(text_templete)
                    self.text_features_test_fra_list.append(self.text_features_test)
                self.text_features_test = torch.stack(self.text_features_test_fra_list)
        ...
        if hasattr(self.args.TRAIN, "TRANSFORMER_DEPTH") and self.args.TRAIN.TRANSFORMER_DEPTH:
            ...
            self.context2 = Transformer_v1(dim=self.mid_dim, heads=8, dim_head_k=self.mid_dim // 8, dropout_atte=0.2)
        #self.context3 = Transformer_v1(dim=self.mid_dim, heads=8, dim_head_k=self.mid_dim // 8, dropout_atte=0.1, depth=1)
        # set_trace()
    ...
    def forward(self, inputs):
        support_images, support_labels, target_images, support_real_class, target_real_class = inputs['support_set'], inputs[
            'support_labels'], inputs['target_set'], inputs['real_support_labels'], inputs['real_target_labels']  # [200, 3, 224, 224] inputs["real_support_labels"]

        # set_trace()
        if self.training:
            #temporal_text_features_train = self.context3(self.text_features_train, self.text_features_train, self.text_features_train)
            s_context_temporal = self.text_features_train[support_real_class.long()]
            #s_context_temporal = self.context3(s_context_temporal, s_context_temporal, s_context_temporal)
            support_features, target_features, _ = self.get_feats(support_images, target_images, support_labels)
            support_bs = support_features.shape[0]
            target_bs = target_features.shape[0]
            if hasattr(self.args.TRAIN, "USE_CLASSIFICATION") and self.args.TRAIN.USE_CLASSIFICATION:
                frame_feature = torch.cat([support_features, target_features], dim=0)
                #feature_classification = self.classification_layer(feature_classification_in).mean(1)
                #class_text_logits = cos_sim(frame_feature, self.text_features_train) * self.scale
                #text_feature = torch.cat([s_context_temporal, self.text_features_train], dim=0)
                # 创建一个用于存储距离的结果张量
                class_text_logits = torch.zeros((frame_feature.size(0), self.text_features_train.size(0))).cuda(device=self.args.gpu_id)
                # 计算每对视频之间的距离
                for i in range(frame_feature.size(0)):  # 对于每个 support 视频
                    for j in range(self.text_features_train.size(0)):  # 对于每个 query 视频
                        # 计算第 i 个 support 视频与第 j 个 query 视频的每一帧之间的余弦相似度
                        cos_similarities = F.cosine_similarity(frame_feature[i], self.text_features_train[j], dim=1)
                        # 计算平均距离
                        average_cos_similarities = cos_similarities.mean()
                        # 存储到距离矩阵中
                        class_text_logits[i, j] = average_cos_similarities
                # frame_feature = rearrange(feature_classification_in, 'b s d -> (b s) d')
                # text_feature = rearrange(st_context_temporal, 'b s d -> (b s) d')
                # 创建一个用于存储余弦相似度的结果张量
                #class_text_logits = torch.zeros((frame_feature.size(0), text_feature.size(0))).cuda(device=self.args.gpu_id, non_blocking=True)

                # 计算每一帧的余弦相似度
                # class_text_logits = []
                # for i in range(frame_feature.size(0)):  # 对于每个样本
                #     for j in range(frame_feature.size(1)):  # 对于每个样本
                #         # 计算第 i 个样本的所有帧与第 j 个样本的所有帧的相似度
                #         #sim_frame = F.cosine_similarity(frame_feature[i], text_feature[j], dim=1)
                #         sim_frame2context = cos_sim(frame_feature[i][j], text_feature) * self.scale
                #         class_text_logits.append(sim_frame2context)

                # frame_sim = cos_sim(frame_feature, text_features)
                # frame_dists = 1 - frame_sim
                #
                # dists = rearrange(frame_dists, '(tb ts) (sb ss) -> tb sb ts ss', tb=feature_classification_in.size(0),
                #                   sb=st_context_temporal.size(0))
                #class_text_logits = OTAM_cum_dist_v2(dists) + OTAM_cum_dist_v2(rearrange(dists, 'tb sb ts ss -> tb sb ss ts'))
            else:
                class_text_logits = None

            target_features = self.context2(target_features, target_features, target_features)
            s_context_temporal = self.mid_layer(s_context_temporal)
            if hasattr(self.args.TRAIN, "MERGE_BEFORE") and self.args.TRAIN.MERGE_BEFORE:
                unique_labels = torch.unique(support_labels)
                support_features = [
                    torch.mean(torch.index_select(support_features, 0, extract_class_indices(support_labels, c)), dim=0)
                ...
                support_features = torch.stack(support_features)
                s_context_temporal = [
                    torch.mean(torch.index_select(s_context_temporal, 0, extract_class_indices(support_labels, c)), dim=0)
                    for c in unique_labels]
                s_context_temporal = torch.stack(s_context_temporal)
            #support_features = torch.cat([support_features, context_support], dim=1)

            support_features_res = support_features + (support_features*s_context_temporal)
            s_context_temporal_res = s_context_temporal + (support_features*s_context_temporal)
            support_features = self.context2(s_context_temporal_res, support_features_res, support_features_res)[:, :self.args.DATA.NUM_INPUT_FRAMES, :]
            s_context_temporal = self.context2(support_features_res, s_context_temporal_res, s_context_temporal_res)[:, :self.args.DATA.NUM_INPUT_FRAMES, :]
            support_features = support_features + (support_features*s_context_temporal)
            ...
            #temporal_text_features_test = self.context3(self.text_features_test, self.text_features_test, self.text_features_test)
            s_context_temporal = self.text_features_test[support_real_class.long()]
            #s_context_temporal = self.context3(s_context_temporal, s_context_temporal, s_context_temporal)
            if hasattr(self.args.TRAIN, "EVAL_TEXT") and self.args.TRAIN.EVAL_TEXT:
                unique_labels = torch.unique(support_labels)
                support_features, target_features, text_features = self.get_feats(support_images, target_images,
                ...
                class_text_logits = torch.zeros((frame_feature.size(0), self.text_features_test.size(0))).cuda(
                    device=self.args.gpu_id)
                # 计算每对视频之间的距离
                for i in range(frame_feature.size(0)):  # 对于每个 support 视频
                    for j in range(self.text_features_test.size(0)):  # 对于每个 query 视频
                        # 计算第 i 个 support 视频与第 j 个 query 视频的每一帧之间的余弦相似度
                        cos_similarities = F.cosine_similarity(frame_feature[i], self.text_features_test[j], dim=1)
                        # 计算平均距离
                        average_cos_similarities = cos_similarities.mean()
                        # 存储到距离矩阵中
                        class_text_logits[i, j] = average_cos_similarities
                ...
                if hasattr(self.args.TRAIN, "MERGE_BEFORE") and self.args.TRAIN.MERGE_BEFORE:
                    ...
                    support_features = torch.stack(support_features)
                    context_support = [
                        torch.mean(torch.index_select(s_context_temporal, 0, extract_class_indices(support_labels, c)),
                                   dim=0) for c in unique_labels]
                    context_support = torch.stack(context_support)
                #support_features = torch.cat([support_features, context_support], dim=1)
                support_features_res = support_features + (support_features * s_context_temporal)
...
@HEAD_REGISTRY.register()
class mpl_few_shot(CNN_FSHead):
    """
    OTAM with a CNN backbone.
    """

    def __init__(self, cfg, feat_dim=2048, num_heads=8):
        super(mpl_few_shot, self).__init__(cfg)
        last_layer_idx = -1
        self.args = cfg
        ...
        with torch.no_grad():
            device = backbone.token_embedding.weight.device
            if hasattr(self.args.TEST, "PROMPT") and self.args.TEST.PROMPT:
                text_templete = [self.args.TEST.PROMPT.format(self.class_real_train[int(ii)]) for ii in
                                 range(len(self.class_real_train))]
            else:
                text_templete = ["a photo of {}".format(self.class_real_train[int(ii)]) for ii in
                                 range(len(self.class_real_train))]
            text_templete = tokenize(text_templete).to(device)
            self.text_features_train = backbone.encode_text(text_templete)
        ...
        # 时序位置编码（显式时间建模）
        self.position_enc = nn.Parameter(torch.randn(1, 8, self.mid_dim))  # (1, T, D)
        # 多头注意力机制（跨样本交互）
        self.mha = nn.MultiheadAttention(embed_dim=self.mid_dim, num_heads=num_heads)

        self.proto_generator = SlidingWindowPrototypeGenerator2(self.mid_dim, 1)
        #self.mse_loss = nn.MSELoss()
        #self.temporal_atte = MultiHeadAttention(8, self.mid_dim, self.mid_dim // 8, self.mid_dim // 8, dropout=0.05)
        self.context2 = Transformer_v1(dim=self.mid_dim, heads=8, dim_head_k=self.mid_dim // 8, dropout_atte=0.1)
        self.layer1 = nn.Sequential(nn.Conv1d(self.mid_dim * 2, self.mid_dim, kernel_size=1, padding=0), )
        #self.lstm_f = VisionLSTM(dim=self.mid_dim, depth=1, pooling="bilateral_avg", output_shape=(self.mid_dim,), mode="feature_extractor")
        #self.lstm_v = VisionLSTM(dim=self.mid_dim, depth=1, pooling="bilateral_avg", output_shape=(self.mid_dim,), mode="feature_extractor")
        max_seq_len = self.args.DATA.NUM_INPUT_FRAMES
        temproal_regular = torch.zeros(max_seq_len, max_seq_len)
        self.mid_layer = nn.Sequential()
        self.classification_layer = nn.Sequential()
    ...
    def sparse_lstm(self, support_features, target_features):
        s1357 = torch.stack([support_features[:, 0, :], support_features[:, 2, :], support_features[:, 4, :], support_features[:, 6, :]], dim=1)
        s2468 = torch.stack([support_features[:, 1, :], support_features[:, 3, :], support_features[:, 5, :], support_features[:, 7, :]], dim=1)
        t1357 = torch.stack([target_features[:, 0, :], target_features[:, 2, :], target_features[:, 4, :],target_features[:, 6, :]], dim=1)
        t2468 = torch.stack([target_features[:, 1, :], target_features[:, 3, :], target_features[:, 5, :],target_features[:, 7, :]], dim=1)

        s1357 = self.relu(self.lstm_f(s1357))
        s2468 = self.relu(self.lstm_f(s2468))
        t1357 = self.relu(self.lstm_f(t1357))
        t2468 = self.relu(self.lstm_f(t2468))
        s_list, t_list = [], []

        for i in range(s1357.size(1)):
            s = torch.cat([s1357[:, i, :].unsqueeze(1), s2468[:, i, :].unsqueeze(1)], dim=1)
            s_list.append(s)
            t = torch.cat([t1357[:, i, :].unsqueeze(1), t2468[:, i, :].unsqueeze(1)], dim=1)
            t_list.append(t)
        s_out = torch.cat([s_list[0], s_list[1], s_list[2], s_list[3]], dim=1)
        t_out = torch.cat([t_list[0], t_list[1], t_list[2], t_list[3]], dim=1)
        # s_out = self.relu(self.lstm_f2(support_features)) + s_out
        # t_out = self.relu(self.lstm_f2(target_features)) + t_out

        return s_out, t_out


    def cstattention(self, class_features):
        """
        Args:
            class_features: (K, T, D) 类内K个样本的时序特征
        Returns:
            class_prototype: (T, D) 增强后的类原型
        """
        K, T, D = class_features.shape
        # 时序位置增强
        features = class_features + self.position_enc  # (K, T, D)

        # 维度调整：多头注意力需要(T, K, D)
        features = features.permute(1, 0, 2)  # (T, K, D)

        # 跨样本注意力交互
        attn_output, _ = self.mha(
            query=features,  # 用时间步作为查询基准
            key=features,
            value=features,
            need_weights=False
        )  # (T, K, D)
        ...
        # 自适应特征融合
        fused_features = self.adaptor(attn_output)  # (T, K, D)

        # 时序原型聚合
        return fused_features.mean(dim=1)  # (T, D)

    def generate_prototypes_with_attention(self, support_features, support_labels):
        """
        使用跨样本时序注意力生成类原型
        Args:
            support_features: (N, T, D) - 支持集特征
            support_labels: (N,) - 支持集标签
        Returns:
            class_prototypes: (C, T, D) - 类原型
        """
        unique_labels = torch.unique(support_labels)
        ...
        for c in unique_labels:
            ...
            class_features = torch.index_select(support_features, 0, class_indices)  # (K, T, D)

            # 应用跨样本时序注意力
            prototype = self.cstattention(class_features)  # (T, D)
            class_prototypes.append(prototype)

        return torch.stack(class_prototypes)  # (C, T, D)
    def get_feats(self, support_images, target_images, support_labels, target_labels, TOP_K, has_VAP=True):
        """
        Takes in images from the support set and query video and returns CNN features.
        """
        support_features = self.backbone(support_images)  # [40, 2048, 7, 7] (5 way - 1 shot - 5 query)
        target_features = self.backbone(target_images)  # [200, 2048, 7, 7]
        dim = int(support_features.shape[1])
        class_text_logits = None
        support_features = support_features.reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dim)
        target_features = target_features.reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dim)
        ...
        s_f_list = []
        t_f_list = []
        # for i in range(len(multi_target_features)):
        #     multi_target_features[i] = multi_target_features[i].reshape(-1, self.args.DATA.NUM_INPUT_FRAMES,multi_target_features[i].size(1))
        #     multi_support_features[i] = multi_support_features[i].reshape(-1, self.args.DATA.NUM_INPUT_FRAMES,multi_support_features[i].size(1)

        #h = support_features.shape[2]
        #dim = int(support_features.shape[1])
        unique_labels = torch.unique(support_labels)
        Query_num = target_features.shape[0]
        frame_num = self.args.DATA.NUM_INPUT_FRAMES
        n_support = support_features.shape[0]

        if TOP_K != 0:
            s_m = rearrange(support_features.unsqueeze(0), 'b h s d -> b (h s) d')
            frame_sim = torch.matmul(F.normalize(s_m, dim=2),
                                     F.normalize(target_features, dim=2).permute(0, 2, 1)).reshape(Query_num, n_support, frame_num, frame_num)
            frame_dists = 1 - frame_sim
            dists = frame_dists
            cum_dists = (dists.min(3)[0].sum(2) + dists.min(2)[0].sum(2))
            #cum_dists_e, _, _ = hausdorff_dist_window(dists, window_size=self.args.DATA.HAUSDORFF_WINDOW_SIZE)
            #cum_dists_e = OTAM_cum_dist(dists) + OTAM_cum_dist(rearrange(dists, 'tb sb ts ss -> tb sb ss ts'))
            values, indices = torch.topk(rearrange(cum_dists, 'c q -> q c'), TOP_K, dim=1, largest=False)
            values_max, indices_max = torch.topk(rearrange(cum_dists, 'c q -> q c'), TOP_K, dim=1, largest=True)
            w_q = F.normalize(values_max-values)
            shot_k_list = []
            for i in range(indices.size(0)):
                shot_list = []
                for j in range(indices.size(1)):
                    shot = target_features[indices[i][j]]
                    shot_list.append(shot*w_q[i][j])
                shot_k = torch.sum(torch.stack(shot_list), dim=0)
                shot_k_list.append(shot_k)
            shot_re = torch.stack(shot_k_list)
            support_features = support_features + shot_re
            class_dists_e = [
                torch.mean(torch.index_select(cum_dists, 1, extract_class_indices(support_labels, c)), dim=1)
                for c in unique_labels]
            class_dists_e = torch.stack(class_dists_e)  # [5, 35]
            class_dists_e = rearrange(class_dists_e, 'c q -> q c')
        else:
            class_dists_e = 0


        # s1 = self.lstm_inf(support_features.reshape(support_features.size(0), dim, -1).permute(0, 2, 1)).permute(0, 2, 1).reshape(-1,dim,h,h)
        # t1 = self.lstm_inf(target_features.reshape(target_features.size(0), dim, -1).permute(0, 2, 1)).permute(0, 2, 1).reshape(-1,dim,h,h)
        # if hasattr(self.args.TRAIN, "USE_CLASSIFICATION") and self.args.TRAIN.USE_CLASSIFICATION:
        #     if hasattr(self.args.TRAIN, "NUM_CLASS"):
        #         class_logits = self.classification_layer(torch.cat([support_features, target_features], 0)).reshape(-1, int(self.args.TRAIN.NUM_CLASS))
        #     else:
        ...
        mean_class_prototypes = [
            torch.mean(torch.index_select(support_features, 0, extract_class_indices(support_labels, c)), dim=0) for c
            in unique_labels]
        mean_class_prototypes = torch.stack(mean_class_prototypes)
        class_prototypes = []
        for c in unique_labels:
            # 提取当前类别的所有样本
            class_indices = extract_class_indices(support_labels, c)
        ...
        s_reshape = torch.stack(class_prototypes)
        #class_prototypes = self.generate_prototypes_with_attention(s, support_labels)
        class_prototypes = self.proto_generator(s_reshape)
        # s22 = self.relu(self.lstm_f(s1))
        # t22 = self.relu(self.lstm_f(t1))
        #s222, t222 = self.sparse_lstm(s1, t1)
        # s22 = self.adaptive_avg_pool(s22.reshape(-1, dim, h, h)).squeeze().reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dim)
        # t22 = self.adaptive_avg_pool(t22.reshape(-1, dim, h, h)).squeeze().reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dim)

        s = mean_class_prototypes + class_prototypes
        # s_out = self.relu(self.lstm_f(s)) + s
        # t_out = self.relu(self.lstm_f(t)) + t


        #s_f_list.append(s1.unsqueeze(0))
        s_f_list.append(s.unsqueeze(0))
        #t_f_list.append(t1)
        t_f_list.append(target_features)

        return s_f_list, t_f_list, class_dists_e, class_text_logits

    def get_v_feat(self, feat, index_tensor):
        mv_list = []
        for k in range(index_tensor.size(0)):
            v_list = []
            for i in range(index_tensor.size(1)):
                result = []
                for j in range(index_tensor.size(2)):
                    f = feat[i][index_tensor[k][i][j]]
                    result.append(f)
                v = torch.mean(torch.stack(result, dim=0), dim=0)
                v_list.append(v)
            s = torch.stack(v_list, dim=0)
            mv_list.append(s)
        return torch.mean(torch.stack(mv_list, dim=0), dim=0)

    def forward(self, inputs):
        support_images, support_labels, target_images, target_labels, real_s_labels, real_t_labels = inputs['support_set'], inputs['support_labels'], inputs['target_set'], inputs['target_labels'], inputs['real_support_labels'], inputs['real_target_labels']  # [200, 3, 224, 224]
        # cur_iter = inputs['cur_iter']

        # if self.training:
        #     context_support = self.text_features_train[real_s_labels.long()].unsqueeze(1)  # .repeat(1, self.args.DATA.NUM_INPUT_FRAMES, 1)
        #
        # else:
        #     context_support = self.text_features_test[real_s_labels.long()].unsqueeze(1)  # .repeat(1, self.args.DATA.NUM_INPUT_FRAMES, 1) # .repeat(support_bs+target_bs, 1, 1)

        top_k = self.args.DATA.TOP_K_TEST

        s_f_list, t_f_list, class_d_e,class_text_logits = self.get_feats(support_images, target_images, support_labels, target_labels, TOP_K=top_k)
        # [35, 5, 8, 2048] [35, 8, 2048] [40, 64]
        unique_labels = torch.unique(support_labels)

        n_queries = t_f_list[0].shape[0]
        n_support = s_f_list[0].shape[1]
        frame_num = s_f_list[0].shape[2]
        dimention = s_f_list[0].shape[-1]

        support_features = s_f_list[0].squeeze()
        support_features_re = rearrange(s_f_list[0], 'b h s d -> b (h s) d')
        target_features = t_f_list[0]# [200, 2048] [35, 40, 2048]

        #support_features = torch.cat([support_features, context_support], dim=1)
        support_features = self.context2(support_features, support_features, support_features)[:,:self.args.DATA.NUM_INPUT_FRAMES, :]

        #frame_sim = cos_sim(target_features, support_features)
        frame_sim = torch.matmul(F.normalize(support_features_re, dim=2),
                                 F.normalize(target_features, dim=2).permute(0, 2, 1)).reshape(n_queries, n_support,
                                                                                               frame_num, frame_num)
        frame_dists = 1 - frame_sim
        dists = frame_dists
        cum_dists_k, dists1, dists2 = hausdorff_dist_window(dists, window_size=self.args.DATA.HAUSDORFF_WINDOW_SIZE)
        class_dists_k = [torch.mean(torch.index_select(cum_dists_k, 1, extract_class_indices(unique_labels, c)), dim=1)
                       for c in unique_labels]
        class_dists_k = torch.stack(class_dists_k)  # [5, 35]
        class_dists_k = rearrange(class_dists_k, 'c q -> q c')
        ##
        s_mean = torch.mean(support_features.reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dimention), dim=1)
        t_mean = torch.mean(target_features, dim=1)
        s_key = self.get_v_feat(support_features.reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dimention),dists1.min(3)[1])
        t_key = self.get_v_feat(target_features.reshape(-1, self.args.DATA.NUM_INPUT_FRAMES, dimention),dists2.min(2)[1].permute(1, 0, 2))
        # s_v = self.relu(self.multi_cross_atte_vid(s_avg.unsqueeze(0), s_avg.unsqueeze(0), s_avg.unsqueeze(0)))+s_mean.unsqueeze(0)
        feature_in = torch.cat([t_key.unsqueeze(0), s_key.unsqueeze(0)], dim=1)
        #feature_out = self.relu(self.lstm_v(feature_in))
        feature_out = self.context2(feature_in,feature_in,feature_in)
        s_key = feature_out[:, -self.args.TRAIN.WAY:, :]
        t_key = feature_out[:, :-self.args.TRAIN.WAY, :]
        #
        s_key = s_key.permute(1, 0, 2) + s_mean.unsqueeze(1)
        t_key = t_key.permute(1, 0, 2) + t_mean.unsqueeze(1)

        support_features = torch.cat([s_key, support_features], dim=1)
        support_features_re = rearrange(support_features.unsqueeze(0), 'b h s d -> b (h s) d')
        target_features = torch.cat([t_key, target_features], dim=1)

        frame_sim = torch.matmul(F.normalize(rearrange(support_features.unsqueeze(0), 'b h s d -> b (h s) d'), dim=2),
                                 F.normalize(target_features, dim=2).permute(0, 2, 1)).reshape(n_queries, n_support,
                                                                                               frame_num+1, frame_num+1)
        frame_dists = 1 - frame_sim
        dists = frame_dists

        #cum_dists, _, _ = hausdorff_dist_window(dists, window_size=self.args.DATA.HAUSDORFF_WINDOW_SIZE)
        cum_dists = (dists.min(3)[0].sum(2) + dists.min(2)[0].sum(2))
        #cum_dists = OTAM_cum_dist(dists) + OTAM_cum_dist(rearrange(dists, 'tb sb ts ss -> tb sb ss ts'))

        class_dists = [torch.mean(torch.index_select(cum_dists, 1, extract_class_indices(unique_labels, c)), dim=1)
                       for c in unique_labels]
        class_dists = torch.stack(class_dists)  # [5, 35]
        class_dists = rearrange(class_dists, 'c q -> q c')
