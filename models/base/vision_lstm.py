...
# Benedikt Alkin, Maximilian Beck, Korbinian P�ppel
import math
...
from models.base.vision_lstm_util import interpolate_sincos, to_ntuple, VitPatchEmbed, VitPosEmbed2d, DropPath
...
class VisionLSTM(nn.Module):
    def __init__(
            ...
            depth=1,
            output_shape=(2048,),
            mode="classifier_and_feature_extractor",
        ...
        elif mode == "classifier" or "classifier_and_feature_extractor":
    ...
    def forward(self, x):
        ...
        if self.pooling is None:
            ...
            x_avg = self.norm(x_avg)
        else:
            raise NotImplementedError(f"pooling '{self.pooling}' is not implemented")

        # head
        if self.head is not None and self.pooling is not None:
            x_avg = self.head(x_avg)
        if self.mode == "feature_extractor":
            return x
        elif self.mode == "classifier_and_feature_extractor":
            return x_avg, x
        else:
            raise NotImplementedError
...
#
# # 创建示例输入张量
# input = torch.randn(5, 8, 2048)
#
# # 前向传播
# output = lstm(input)
#
# print(output.shape)  # 输出: torch.Size([3, 20])