...
def train_epoch(
    ...
    for cur_iter, task_dict in enumerate(train_loader):
        # task_dict["cur_iter"] = torch.tensor([cur_iter])
        # task_dict["mode"] = torch.tensor([mmode])
        ...
        if (cur_iter + 1) % cfg.TRAIN.VAL_FRE_ITER == 0:   # 
            ...
            top1_err_eval = eval_epoch(val_loader, model, val_meter,
                                                      cur_epoch_save + cfg.TRAIN.NUM_FOLDS - 1, cfg, )
            if best_top1_err > top1_err_eval:
                cu.save_checkpoint(cfg.OUTPUT_DIR, model, model_ema, optimizer,
                                   cur_epoch_save + cfg.TRAIN.NUM_FOLDS - 1, cfg, model_bucket)
                best_top1_err = top1_err_eval
        ...
        # if misc.get_num_gpus(cfg):
        #     for k in task_dict.keys():
        #         task_dict[k] = task_dict[k][0].cuda(device=cfg.gpu_id[0], non_blocking=True)
        # if misc.get_num_gpus(cfg):
        #     for idx, (k, v) in enumerate(task_dict.items()):
        #         device = cfg.gpu_id[idx % len(cfg.gpu_id)]  # 循环分配到多个GPU
        #         task_dict[k] = v[0].cuda(device=device, non_blocking=True)
        #
        ...
        # divergence_loss = model_dict['divergence_loss']
        # clip_loss = model_dict['clip_loss']
        ...
        if hasattr(cfg.TRAIN,"USE_CLASSIFICATION") and cfg.TRAIN.USE_CLASSIFICATION:
            if hasattr(cfg.TRAIN,"USE_CLASSIFICATION_ONLY") and cfg.TRAIN.USE_CLASSIFICATION_ONLY:
                ...
                if hasattr(cfg.TRAIN,"USE_CONTRASTIVE") and cfg.TRAIN.USE_CONTRASTIVE:
                    ...
                    # + 0.2*F.cross_entropy(model_dict["class_d_e"], task_dict["target_labels"].long())
                    # + cfg.TRAIN.USE_CLASSIFICATION_VALUE * F.cross_entropy(model_dict["class_logits"], torch.cat([task_dict["real_support_labels"], task_dict["real_target_labels"]], 0).unsqueeze(1).repeat(1,cfg.DATA.NUM_INPUT_FRAMES).reshape(-1).long())
                    ...
                    #
                            #+ cfg.TRAIN.USE_CLASSIFICATION_VALUE *F.cross_entropy(model_dict["class_logits"],torch.cat([task_dict["real_support_labels"], task_dict["real_target_labels"]], 0).unsqueeze(1).repeat(1,cfg.DATA.NUM_INPUT_FRAMES).reshape(-1).long())) /cfg.TRAIN.BATCH_SIZE  \
                            # + divergence_loss \
                            # + 0.1*clip_loss
        else:
...
@torch.no_grad()
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg, writer=None, mmode=1):
    """
    ...
    # Log epoch stats.
    top1_err_show = val_meter.log_epoch_stats(cur_epoch)
